...
public class VoldemortClientShell {
    ...
    public static void main(String[] args) throws Exception {
        ...
        OptionSpec<Boolean> proxyEnableOptionSpec = parser.accepts("proxy-enable", "is proxy used?")
                .withOptionalArg()
                .ofType(Boolean.class)
                .defaultsTo(false);
        OptionSpec<String> proxyBasePortNumberSpec = parser.accepts("proxy-base-local-port")
                .withOptionalArg()
                .ofType(String.class);
        OptionSpec<Boolean> predDetectionOption = parser.accepts("predicate-detection", "enable predicate detection or not")
                .withOptionalArg()
    ...
    protected void processCommands(boolean printCommands) throws IOException {
        for(String line = commandReader.readLine(); line != null; line = commandReader.readLine()) {
            ...
            long evalutationTime = System.nanoTime() - startTimeNs;
            commandOutput.println("-- Time to complete command: ");
            commandOutput.println("      " + evalutationTime + " nano-seconds");
            commandOutput.println("      " + (evalutationTime/ SystemTime.NS_PER_MS) + " milliseconds");
