...
import voldemort.VoldemortClientGraph;
import voldemort.VoldemortClientMeasurePerformance;
    ...
    private String bootstrapMetadata(String key, URI[] urls) {
        for(URI url: urls) {
            try {
                ...
                if(found.size() == 1) {
                    // Duong change
                    // we add this change to accommodate the proxy at the client
                    // originally, the code is simply return the first string in the List
                    // return found.get(0).getValue();
                    // now we will check if proxy is enabled or not, and modify the string accordingly

                    String originalBootstrapMetadata = found.get(0).getValue();
                    String modifiedBootstrapMetadata = originalBootstrapMetadata;

//                    if(VoldemortClientMeasurePerformance.proxyIsEnabled){
//                        modifiedBootstrapMetadata =
//                                VoldemortClientMeasurePerformance.replaceServersByProxies(originalBootstrapMetadata);
//                    }

                    if(ClientProxy.isProxyIsEnabled()){
                        modifiedBootstrapMetadata =
                                ClientProxy.replaceServersByProxies(originalBootstrapMetadata);
                    }


                    System.out.println("\n--------------------");
                    System.out.println(" the originalBootStrapMetadata is: ");
                    System.out.println(originalBootstrapMetadata);
                    System.out.println("\n");
                    System.out.println(" modifiedBootStrapMetadata is:");
                    System.out.println(modifiedBootstrapMetadata);
                    System.out.println("\n--------------------");

                    return modifiedBootstrapMetadata;
                }
            } catch(Exception e) {
